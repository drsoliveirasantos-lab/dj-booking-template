...
function bookingMessage(form){const d=new FormData(form);return [`Hello ${ARTIST.name},`,`I would like to check your availability for an event.`,``,`Name: ${d.get('name')||'-'}`,`Phone/WhatsApp: ${d.get('phone')||'-'}`,`Event type: ${d.get('eventType')||'-'}`,`City: ${d.get('city')||'-'}`,`Date: ${d.get('date')||'-'}`,`Approx. guests: ${d.get('guests')||'-'}`,`Approx. budget: ${d.get('budget')||'-'}`,`Message: ${d.get('message')||'-'}`].join('\n')}
function setup(){
